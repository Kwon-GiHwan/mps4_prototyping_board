"""Generate PMU_INTERVAL_FINE_DIAG_V10 sources from frozen runner/vendor inputs."""
...
def patch_runner(text: str) -> tuple[str, dict]:
    ...
    text, counts["schema_version_branch"] = sub_once(
        ...
        "#if defined(PMU_INTERVAL_FINE_DIAG_V10)\n#define PMU_DIAG_SCHEMA_VERSION 10U\n"
    ...
    text, counts["private_driver_boundary"] = sub_once(
        ...
        "#endif",
        "#if defined(PMU_DIAG_USES_PRIVATE_DRIVER) && !defined(PMU_INTERVAL_FINE_DIAG_V10)\n"
        "#error \"PMU_QUAL: schema v8 must link the reference vendor u85.c\"\n"
        "#endif",
        "v10 private-driver boundary",
    )
    text, counts["s1_s2_private_driver_boundary"] = sub_once(
        ...
        "#endif\n"
        "#endif",
        "v10 S1/S2 private-driver boundary",
    )
    ...
    text, counts["field_count_block"] = sub_once(
        ...
        "#if defined(PMU_INTERVAL_FINE_DIAG_V10)\n"
        "#define PMU_DIAG_FIELD_COUNT (40U + 13U + (4U * PMU_DIAG_SNAPSHOT_WORDS) + 6U)\n"
        "#elif defined(PMU_QUAL_SCHEMA_V8)\n"
        "#define PMU_DIAG_FIELD_COUNT (40U + 13U + (4U * PMU_DIAG_SNAPSHOT_WORDS))\n"
        "#else\n#define PMU_DIAG_FIELD_COUNT (40U + (3U * PMU_DIAG_SNAPSHOT_WORDS))\n#endif",
        "field count block",
    ...
    text, counts["static_asserts"] = sub_once(
        ...
        "#if defined(PMU_INTERVAL_FINE_DIAG_V10)\n"
        "_Static_assert(sizeof(pmu_diag_snapshot_t) == PMU_DIAG_SNAPSHOT_WORDS * 4U,\n"
        "               \"PMU_INTERVAL_FINE_DIAG_V10: snapshot must be 8 words\");\n"
        "_Static_assert(PMU_DIAG_FIELD_COUNT == 91U,\n"
        "               \"PMU_INTERVAL_FINE_DIAG_V10: v8 body plus six fields\");\n"
        "_Static_assert(PMU_DIAG_TOTAL_WORDS == 99U,\n"
        "               \"PMU_INTERVAL_FINE_DIAG_V10: 8 header plus 91 body\");\n"
        "_Static_assert(PMU_DIAG_PAYLOAD_SIZE == 396U,\n"
        "               \"PMU_INTERVAL_FINE_DIAG_V10: payload is 396 bytes\");\n"
        "_Static_assert(PMU_DIAG_SCHEMA_VERSION == 10U,\n"
        "               \"PMU_INTERVAL_FINE_DIAG_V10: schema must be 10\");\n"
        "#elif defined(PMU_QUAL_SCHEMA_V8)\n/* The wire shape, asserted at compile time rather than trusted. The host\n"
        " * refuses anything that is not exactly 93 words / 372 bytes, so a mismatch\n"
        " * here would otherwise surface as an unparseable board run. */\n"
        "_Static_assert(sizeof(pmu_diag_snapshot_t) == PMU_DIAG_SNAPSHOT_WORDS * 4U,\n"
...
def patch_vendor(text: str) -> tuple[str, dict]:
    ...
    text, counts["irq_post_t3_counts"] = sub_once(
        text, irq_tail, irq_tail_v10, "post-T3 ISR hit counters")
    return text, counts
