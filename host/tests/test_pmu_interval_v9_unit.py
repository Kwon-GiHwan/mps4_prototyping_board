...
import analyze_pmu_interval_v9 as az
import run_pmu_interval_v9 as rv9
...
def manifest(**over):
    doc = {
        ...
        "schema_version": 9,
        "build_id": "0x%08X" % v9.PMU_INTERVAL_DIAG_V9_BUILD_ID,
...
def build(**over):
    ...
    prefix = [
        9,
        v9.PMU_INTERVAL_DIAG_V9_BUILD_ID,
        1, 0, 7,
        0, 0, 0,
    ...
    total = v9.PMU_INTERVAL_DIAG_V9_HEADER_WORDS + len(body)
    head = struct.pack("<8I", v9.PMU_INTERVAL_DIAG_V9_MAGIC, 9, total,
                       v9.PMU_INTERVAL_DIAG_V9_HEADER_WORDS, prefix[4], prefix[9], prefix[8], 0)
...
def archive_doc(raw, man=None, host_boot_index=1):
    ...
    parsed = v9.parse_pmu_interval_diag_v9_payload(raw)
    return {
        "variant": "PMU_INTERVAL_DIAG_V9",
        "host": {
        ...
        "target": v9.target_fields(parsed),
        "derived": v9.classify_pmu_interval_diag_v9(parsed, man),
        "raw": {
            "payload_hex": raw.hex(),
...
def campaign_payload(boot, repeat, primary_window):
    ...
    d12 = 20
    d23 = 30
    d34 = primary_window - (d01 + d12 + d23)
    d45 = 50
    return build(
        pre=snap(cyc=1000),
...
print("=== ABI ===")
p = build()
res = v9.parse_pmu_interval_diag_v9_payload(p)
check("v9 payload size is 384 bytes", len(p) == v9.PMU_INTERVAL_DIAG_V9_PAYLOAD_SIZE)
check("v9 parser decodes extra checkpoints",
      (res.t_submit_before_cmd, res.t_submit_after_cmd, res.t_irq_status_seen) == (200, 300, 500))
try:
    v8.parse_pmu_qual_payload(p)
    check("v8 parser rejects v9 payload", False)
except v8.ProtocolError:
    check("v8 parser rejects v9 payload", True)

bad = bytearray(p)
bad[40] ^= 0x01
try:
    v9.parse_pmu_interval_diag_v9_payload(bytes(bad))
    check("CRC mismatch rejected", False)
except v8.ProtocolError:
    check("CRC mismatch rejected", True)

print("=== classification ===")
cls = v9.classify_pmu_interval_diag_v9(res, manifest())
check("valid sample classifies", cls["valid"], "reasons=%s" % cls["invalid_reasons"])
check("D23 exported", cls["deltas_u32"]["D23"] == 200)
check("characterization labels present",
      cls["busy_poll_interval_only"] and cls["not_a_latency_measurement"])
...
check("V9 window has a non-comparable V9-only name",
      cls["v9_perturbed_window_cycles"] is not None
      and cls["not_comparable_to_v8_cfg_window"] is True
      and "npu_pmu_window_cycles" not in cls)

try:
    v9.verify_manifest_identity(manifest(production_end_only_frozen=False), "test")
    check("Production-unfrozen manifest rejected", False)
except SystemExit:
    check("Production-unfrozen manifest rejected", True)

try:
    v9.verify_manifest_identity(manifest(artifact_sha256={}), "test")
    check("empty artifact provenance rejected", False)
except SystemExit:
    check("empty artifact provenance rejected", True)

cls = v9.classify_pmu_interval_diag_v9(
    v9.parse_pmu_interval_diag_v9_payload(build(t_submit_after_cmd=200)),
    manifest(),
)
check("non-monotonic checkpoints fail closed",
      (not cls["valid"]) and "checkpoint_order_u32" in cls["invalid_reasons"])

cls = v9.classify_pmu_interval_diag_v9(
    v9.parse_pmu_interval_diag_v9_payload(build(t_irq_status_seen=0)),
    manifest(),
)
...
for label, override, reason in (
    ...
    cls = v9.classify_pmu_interval_diag_v9(
        v9.parse_pmu_interval_diag_v9_payload(build(**override)), manifest())
    check(label + " fails closed",
          (not cls["valid"]) and reason in cls["invalid_reasons"])

try:
    v9.parse_pmu_interval_diag_v9_payload(build(power_rehold_performed=1))
    check("retained rehold slot rejected by parser", False)
except v8.ProtocolError:
    check("retained rehold slot rejected by parser", True)

cls = v9.classify_pmu_interval_diag_v9(
    v9.parse_pmu_interval_diag_v9_payload(build(build_id=0xDEADBEEF)),
    manifest(),
)
check("wrong build id fails closed",
      (not cls["valid"]) and "build_id_is_v9" in cls["invalid_reasons"])

cls = v9.classify_pmu_interval_diag_v9(
    v9.parse_pmu_interval_diag_v9_payload(build(hook_callsite_lr_observed=LR + 4)),
    manifest(),
)
...
cls = v9.classify_pmu_interval_diag_v9(
    v9.parse_pmu_interval_diag_v9_payload(build(golden_window_crc=0xDEAD)),
    manifest(),
)
...
res, raw, reread_raw = rv9.collect_pmu_interval_v9(fake_link(p), timeout=0.01, get_timeout=0.01)
check("ACK/COMPLETE/GET exchange accepts byte-identical v9", raw == reread_raw == p)
...
for label, link in (
    ...
    try:
        rv9.collect_pmu_interval_v9(link, timeout=0.01, get_timeout=0.01)
        check(label, False)
...
_, raw, _ = rv9.collect_pmu_interval_v9(link, timeout=0.01, get_timeout=0.01)
check("stale prior-sequence frame skipped", raw == p and link.late_frames == 1)

try:
    rv9.verify_record_identity(
        v9.parse_pmu_interval_diag_v9_payload(build(diag_case=2)), manifest())
    check("wrong V9 case identity rejected before archive", False)
except SystemExit:
    check("wrong V9 case identity rejected before archive", True)
...
with tempfile.TemporaryDirectory() as td:
    ...
    check("analyzer reloads and re-derives sample", loaded["variant"] == "PMU_INTERVAL_DIAG_V9")
...
with tempfile.TemporaryDirectory() as td:
    ...
    check("earliest excursion interval localizes to D34",
          report["floor_excursion"]["earliest_interval_counts"] == {"D34": 10})
    stats = report["v9_perturbed_window_stats"]
    check("overall V9 window stats are derived",
    ...
    check("per-boot D34 stats are split correctly",
          report["per_boot_delta_stats"]["1"]["D34"]["min"] == 40
          and report["per_boot_delta_stats"]["2"]["D34"]["max"] == 70
          and report["per_boot_delta_stats"]["3"]["D34"]["max"] == 100)
    check("constant interval CV collapses to zero",
          report["delta_stats"]["D01"]["cv"] == 0.0)
