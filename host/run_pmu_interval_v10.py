"""Collect one PMU_INTERVAL_FINE_DIAG_V10 sample and bind it to its manifest."""
...
from runner_proto_pmu_interval_v10 import (
    PMU_INTERVAL_FINE_DIAG_V10_BUILD_ID,
...
def read_manifest(path: str) -> tuple[dict, bytes]:
    ...
    if digest != PMU_INTERVAL_V10_FROZEN_MANIFEST_SHA256:
        raise SystemExit(
            "FAIL %s: manifest SHA-256 %s does not match frozen V10 %s"
            % (path, digest, PMU_INTERVAL_V10_FROZEN_MANIFEST_SHA256))
    verify_manifest_identity(doc, path)
    return doc, blob


def verify_record_identity(res, manifest: dict) -> None:
    if res.build_id != PMU_INTERVAL_FINE_DIAG_V10_BUILD_ID:
        raise SystemExit(
            "FAIL target build_id 0x%08X is not the v10 identity 0x%08X"
            % (res.build_id, PMU_INTERVAL_FINE_DIAG_V10_BUILD_ID)
        )
    ...
    if res.diag_case != 1 or res.nc_control_id != 0:
        raise SystemExit(
            "FAIL target identity diag_case=%d nc_control_id=%d is not V10 normal case-A"
...
def _read_v10_result(link: rq.PmuQualLink, timeout: float) -> bytes:
...
def collect_pmu_interval_v10(
    ...
    reread = _read_v10_result(link, get_timeout)
    if not reread:
        raise RunSequenceError("GET_PMU_DIAG_RESULT returned an empty payload")
    if reread != raw:
        raise RunSequenceError(
            "GET_PMU_DIAG_RESULT reread differs from COMPLETE payload")
    parse_pmu_interval_diag_v10_payload(reread)
    link.last_pmu_diag_reread_raw = reread
    return res, raw, reread


def build_record(manifest: dict, manifest_blob: bytes, artifacts: dict, res, raw: bytes, reread: bytes,
                 host_boot_index: int, manifest_path: str) -> dict:
...
def main() -> None:
    ...
    try:
        link.ping()
        rq.prime(link)
        res, raw, reread = collect_pmu_interval_v10(link)
    finally:
        link.close()
    ...
    if not doc["derived"]["valid"]:
        raise SystemExit("FAIL invalid V10 sample archived without a V10 window value")
