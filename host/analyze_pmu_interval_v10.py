"""Re-derive PMU_INTERVAL_FINE_DIAG_V10 archives.
...
from runner_proto_pmu_interval_v10 import (
    PMU_INTERVAL_ANALYZER_PROHIBITED_CLAIMS,
    PMU_INTERVAL_V10_FROZEN_MANIFEST_SHA256,
    classify_pmu_interval_diag_v10,
    parse_pmu_interval_diag_v10_payload,
    target_fields,
...
REPORT_INTERVALS = ("E0", "E1", "D23")
LOCALIZATION_INTERVALS = ("E0", "E1")
PRIMARY_WINDOW = "v10_perturbed_window_cycles"
...
def load(
    path: str,
    expected_manifest_sha256: str = PMU_INTERVAL_V10_FROZEN_MANIFEST_SHA256,
) -> tuple[object, dict]:
    with open(path) as handle:
        doc = json.load(handle)
    if doc.get("variant") != "PMU_INTERVAL_FINE_DIAG_V10":
        raise SystemExit("FAIL %s: not a PMU_INTERVAL_FINE_DIAG_V10 archive" % path)
    ...
    if observed_manifest_sha256 != expected_manifest_sha256:
        raise SystemExit(
            "FAIL %s: manifest SHA-256 %s does not match frozen V10 %s"
    ...
    res = parse_pmu_interval_diag_v10_payload(raw)
    derived = classify_pmu_interval_diag_v10(res, manifest)
...
def _campaign_rows(paths: list[str], expected_manifest_sha256: str) -> list[dict]:
    ...
    for path in sorted(paths):
        ...
        derived = classify_pmu_interval_diag_v10(res, doc["manifest"])
...
def summarize_campaign(
    ...
    for boot in boots:
        ...
        if got_seq != want_seq:
            raise SystemExit(
                "FAIL boot %d run_sequence %s, expected %s"
                % (boot, got_seq, want_seq)
            )
    ...
    if any(value is None for value in window_values):
        raise SystemExit("FAIL campaign contains an unpublished V10 window value")
    floor_value = min(window_values)
    floor_rows = [row for row in rows if row["derived"][PRIMARY_WINDOW] == floor_value]
    excursion_rows = [
        row for row in rows if row["derived"][PRIMARY_WINDOW] > floor_value
    ]
    floor_ranges = {
        name: (
    ...
    return {
        ...
        "delta_stats": overall,
        "v10_perturbed_window_stats": _series_stats(window_values),
...
def main() -> int:
    ...
    if len(args.path) == 1:
        ...
        derived = classify_pmu_interval_diag_v10(res, doc["manifest"])
        print("variant: PMU_INTERVAL_FINE_DIAG_V10")
        ...
        print("t2_t3_label: %s" % derived["t2_t3_label"])
        print("t2_i0_label: %s" % derived["t2_i0_label"])
        print("i0_t3_label: %s" % derived["i0_t3_label"])
    ...
    print("primary_window_floor_excursion: %s" % json.dumps(report["floor_excursion"], sort_keys=True))
