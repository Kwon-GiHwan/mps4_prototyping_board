"""Collect one PMU_INTERVAL_DIAG_V9 sample and bind it to its manifest."""
...
def read_manifest(path: str) -> tuple[dict, bytes]:
    ...
    if digest != PMU_INTERVAL_V9_FROZEN_MANIFEST_SHA256:
        raise SystemExit(
            "FAIL %s: manifest SHA-256 %s does not match frozen V9 %s"
            % (path, digest, PMU_INTERVAL_V9_FROZEN_MANIFEST_SHA256))
    verify_manifest_identity(doc, path)
    return doc, blob


def verify_record_identity(res, manifest: dict) -> None:
    if res.build_id != PMU_INTERVAL_DIAG_V9_BUILD_ID:
        raise SystemExit(
            "FAIL target build_id 0x%08X is not the v9 identity 0x%08X"
            % (res.build_id, PMU_INTERVAL_DIAG_V9_BUILD_ID)
        )
    ...
    if res.diag_case != 1 or res.nc_control_id != 0:
        raise SystemExit(
            "FAIL target identity diag_case=%d nc_control_id=%d is not V9 normal case-A"
...
def _read_v9_result(link: rq.PmuQualLink, timeout: float) -> bytes:
...
def collect_pmu_interval_v9(
    ...
    reread = _read_v9_result(link, get_timeout)
    if not reread:
    ...
    parse_pmu_interval_diag_v9_payload(reread)
...
def main() -> None:
    ...
    try:
        link.ping()
        rq.prime(link)
        res, raw, reread = collect_pmu_interval_v9(link)
    finally:
        link.close()
    ...
    if not doc["derived"]["valid"]:
        raise SystemExit("FAIL invalid V9 sample archived without a V9 window value")
