"""Re-derive PMU_INTERVAL_DIAG_V9 archives.
...
from runner_proto_pmu_interval_v9 import (
    PMU_INTERVAL_ANALYZER_PROHIBITED_CLAIMS,
    PMU_INTERVAL_V9_FROZEN_MANIFEST_SHA256,
    classify_pmu_interval_diag_v9,
    parse_pmu_interval_diag_v9_payload,
    target_fields,
...
ADJACENT_INTERVALS = ("D01", "D12", "D23", "D34", "D45")
PRIMARY_WINDOW = "v9_perturbed_window_cycles"
...
def load(
    path: str,
    expected_manifest_sha256: str = PMU_INTERVAL_V9_FROZEN_MANIFEST_SHA256,
) -> tuple[object, dict]:
    with open(path) as handle:
        doc = json.load(handle)
    if doc.get("variant") != "PMU_INTERVAL_DIAG_V9":
        raise SystemExit("FAIL %s: not a PMU_INTERVAL_DIAG_V9 archive" % path)
    ...
    if observed_manifest_sha256 != expected_manifest_sha256:
        raise SystemExit(
            "FAIL %s: manifest SHA-256 %s does not match frozen V9 %s"
    ...
    res = parse_pmu_interval_diag_v9_payload(raw)
    derived = classify_pmu_interval_diag_v9(res, manifest)
...
def _campaign_rows(paths: list[str], expected_manifest_sha256: str) -> list[dict]:
    ...
    for path in sorted(paths):
        ...
        derived = classify_pmu_interval_diag_v9(res, doc["manifest"])
...
def summarize_campaign(
    ...
    delta_names = list(ADJACENT_INTERVALS)
    for row in rows[1:]:
    ...
    if any(value is None for value in window_values):
        raise SystemExit("FAIL campaign contains an unpublished V9 window value")
    ...
    earliest_counts = {name: 0 for name in delta_names}
    unresolved = []
    ...
    return {
        ...
        "delta_stats": overall,
        "v9_perturbed_window_stats": _series_stats(window_values),
...
def main() -> int:
    ...
    if len(args.path) == 1:
        ...
        derived = classify_pmu_interval_diag_v9(res, doc["manifest"])
        print("variant: PMU_INTERVAL_DIAG_V9")
