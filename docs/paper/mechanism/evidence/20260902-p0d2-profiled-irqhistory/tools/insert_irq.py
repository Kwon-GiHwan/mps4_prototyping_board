...
def main():
    ...
    ap.add_argument("--all", action="store_true",
                    help="insert after EVERY non-DMA op launch (param = 1-based seq)")
    ap.add_argument("--onehot", action="store_true",
                    help="with --all: param = 1 << (seq %% 16) for irq-history attribution")
    a = ap.parse_args()
    ...
    if a.all:
        launches = [i for i, (ctrl, opc, _, _) in enumerate(cmds)
                    if ctrl == 0 and CMD0[opc] in OP_LAUNCH]
        if not launches:
            die("no op launches found")
        newc = list(cmds)
        for seq, li in reversed(list(enumerate(launches))):
            par = (1 << (seq % 16)) if a.onehot else seq + 1
            newc.insert(li + 1, (0, IRQ_OPC, par, None))
        new_cms = serialize_cms(newc)
        assert len(new_cms) == len(cms) + 4 * len(launches)
        ...
        assert len(c2) - len(stripped) == len(launches)
        open(a.dst, "wb").write(out)
        h = lambda x: hashlib.sha256(x).hexdigest()
        print("MODE: IRQ-ALL x%d (every non-DMA op launch)" % len(launches))
        print("src_tflite_sha256:", h(src)); print("dst_tflite_sha256:", h(out))
        print("orig_cms_sha256:", h(cms)); print("mod_cms_sha256:", h(new_cms))
        print("VERIFY: PASS")
        return
