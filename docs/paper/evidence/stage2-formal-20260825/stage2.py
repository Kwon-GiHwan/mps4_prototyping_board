"""Formal FVP Stage 2 — repetition #2 across the same 74 cells.

Same frozen anchor, same canonical order, same gates. Adds the determinism
test: the equality-bearing vector fixed in DETERMINISTIC_METRIC_VECTOR.md must
match Stage 1 exactly. Any mismatch stops the stage and forbids Stage 3 - never
averaged, re-run, or out-voted by the qualification value.
"""
...
def determinism_diff(m1, cur_meas, cur_art, cur_cfg):
    """Exact equality across the fixed vector. Operational telemetry excluded."""
    bad = {}
    for k in EQUALITY_MEASUREMENT_FIELDS:
        if m1["M1"].get(k) != cur_meas.get(k):
            bad["measurement." + k] = [m1["M1"].get(k), cur_meas.get(k)]
    for k in EQUALITY_ARTIFACT_FIELDS:
        if m1["artifact_identity"].get(k) != cur_art.get(k):
            bad["artifact_identity." + k] = [m1["artifact_identity"].get(k), cur_art.get(k)]
    for k in EQUALITY_CONFIG_FIELDS:
        if m1["config_identity"].get(k) != cur_cfg.get(k):
            bad["config_identity." + k] = [m1["config_identity"].get(k), cur_cfg.get(k)]
    return bad


def stop(outdir, payload):
    json.dump(payload, open(os.path.join(outdir, "_STAGE2_STOP.json"), "w"), indent=1)
    print("STAGE2_STOP %s" % json.dumps({k: payload[k] for k in ("seq", "cell_id", "reason")}),
          flush=True)
...
def main():
    ...
    outdir = sys.argv[2]
    frozen = json.load(open(sys.argv[3]))
    m1_by_seq = {r["seq"]: r for r in frozen["records"]}
    ...
    for cell in cells:
        ...
        r = run_once(cell, b["axf"], os.path.join(b["ws"], "uart_m2.txt"),
                     os.path.join(b["ws"], "fvp_m2.log"))
        shutil.rmtree(b["ws"], ignore_errors=True)
        ...
        rec = {"seq": cell["seq"], "cell_id": cell["cell_id"], "repetition": 2,
               "formal_sample": "M2",
               "gate": checks,
               "artifact_identity": {
        ...
        cur_cfg = {"platform": cell["platform"], "npu": cell["npu"],
                   ...
                   "embedded_build_stamp": b["embedded_build_stamp"],
                   "source_date_epoch": EPOCH}
        diff = determinism_diff(m1_by_seq[cell["seq"]], rec["measurement"],
                                rec["artifact_identity"], cur_cfg)
        rec["determinism"] = {"M1_equals_M2": not diff, "mismatched_fields": diff}
        if diff:
            stop(outdir, {"seq": cell["seq"], "cell_id": cell["cell_id"],
                          "reason": "DETERMINISM_FAILURE", "mismatched_fields": diff})
            return 5
        json.dump(rec, open(dest, "w"), indent=1)
        print("[%2d/74] %-48s M2 total=%-10s M1==M2 wall=%6.2fs %4.0fs" %
              (cell["seq"], cell["cell_id"][:48], r["npu_total_cycles"],
               r["wall_clock_s"], rec["elapsed_s"]), flush=True)
    print("STAGE2_COMPLETE", flush=True)
    return 0
