"""Formal FVP Stage 1 — repetition #1 across the 74 primary executable cells.

Every cell must byte-reproduce its anchored artifacts before it is allowed to
run. Any hard-stop condition halts the whole stage; there is no top-up and no
discretionary retry. No analysis is performed here.
"""
...
def stop(outdir, payload):
    json.dump(payload, open(os.path.join(outdir, "_STAGE1_STOP.json"), "w"), indent=1)
    print("STAGE1_STOP %s" % json.dumps({k: payload[k] for k in ("seq", "cell_id", "reason")}),
          flush=True)
...
def main():
    ...
    for cell in cells:
        ...
        r = run_once(cell, b["axf"], os.path.join(b["ws"], "uart_m1.txt"),
                     os.path.join(b["ws"], "fvp_m1.log"))
        shutil.rmtree(b["ws"], ignore_errors=True)
        ...
        rec = {"seq": cell["seq"], "cell_id": cell["cell_id"], "repetition": 1,
               "formal_sample": "M1",
               "gate": checks,
               "artifact_identity": {
        ...
        print("[%2d/74] %-48s M1 total=%-10s wall=%6.2fs %4.0fs" %
