...
#if (defined(PMU_DIAG_SEAM_S1) || defined(PMU_DIAG_SEAM_S2)) && !defined(PMU_QUAL_SCHEMA_V15)
...
#if defined(PMU_DIAG_USES_PRIVATE_DRIVER) && !defined(PMU_QUAL_SCHEMA_V15)
...
typedef struct {
    ...
    uint32_t pre_program_status;
    uint32_t pre_submit_status;
    uint32_t t_submit_after_cmd;
    uint32_t t_primary_entry;
    uint32_t t_first_observation;
    uint32_t primary_result;
    uint32_t primary_iterations;
    uint32_t first_qread;
    uint32_t first_status;
    uint32_t first_q_done;
    uint32_t first_cmd_end_reached;
    uint32_t first_irq_raised;
    uint32_t first_state;
    uint32_t convergence_result;
    uint32_t convergence_iterations;
    uint32_t convergence_final_qread;
    uint32_t convergence_final_status;
    uint32_t convergence_timeout;
    uint32_t failure_phase;
    uint32_t failure_reason;
    uint32_t failure_qread;
...
#if defined(PMU_QUAL_SCHEMA_V15)
    put32(&c, d->variant_id);
    put32(&c, d->qsize_expected);
    put32(&c, d->pre_program_status);
    put32(&c, d->pre_submit_status);
    put32(&c, d->t_submit_after_cmd);
    put32(&c, d->t_primary_entry);
    put32(&c, d->t_first_observation);
    put32(&c, d->primary_result);
    put32(&c, d->primary_iterations);
    put32(&c, d->first_qread);
    put32(&c, d->first_status);
    put32(&c, d->first_q_done);
    put32(&c, d->first_cmd_end_reached);
    put32(&c, d->first_irq_raised);
    put32(&c, d->first_state);
    put32(&c, d->convergence_result);
    put32(&c, d->convergence_iterations);
    put32(&c, d->convergence_final_qread);
    put32(&c, d->convergence_final_status);
    put32(&c, d->convergence_timeout);
    put32(&c, d->failure_phase);
    put32(&c, d->failure_reason);
    put32(&c, d->failure_qread);
    put32(&c, d->failure_status);
...
# endif
        ...
        if (pmu_completion_visibility_v15_mailbox[33] != V15_MAILBOX_VALID) {
            ...
            d.variant_id = pmu_completion_visibility_v15_mailbox[0];
            d.qsize_expected = pmu_completion_visibility_v15_mailbox[1];
            d.pre_program_status = pmu_completion_visibility_v15_mailbox[2];
            d.pre_submit_status = pmu_completion_visibility_v15_mailbox[3];
            d.t_submit_after_cmd = pmu_completion_visibility_v15_mailbox[4];
            d.t_primary_entry = pmu_completion_visibility_v15_mailbox[5];
            d.t_first_observation = pmu_completion_visibility_v15_mailbox[6];
            d.primary_result = pmu_completion_visibility_v15_mailbox[7];
            d.primary_iterations = pmu_completion_visibility_v15_mailbox[8];
            d.first_qread = pmu_completion_visibility_v15_mailbox[9];
            d.first_status = pmu_completion_visibility_v15_mailbox[10];
            d.first_q_done = pmu_completion_visibility_v15_mailbox[11];
            d.first_cmd_end_reached = pmu_completion_visibility_v15_mailbox[12];
            d.first_irq_raised = pmu_completion_visibility_v15_mailbox[13];
            d.first_state = pmu_completion_visibility_v15_mailbox[14];
            d.convergence_result = pmu_completion_visibility_v15_mailbox[15];
            d.convergence_iterations = pmu_completion_visibility_v15_mailbox[16];
            d.convergence_final_qread = pmu_completion_visibility_v15_mailbox[17];
            d.convergence_final_status = pmu_completion_visibility_v15_mailbox[18];
            d.convergence_timeout = pmu_completion_visibility_v15_mailbox[19];
            d.failure_phase = pmu_completion_visibility_v15_mailbox[20];
            d.failure_reason = pmu_completion_visibility_v15_mailbox[21];
            d.failure_qread = pmu_completion_visibility_v15_mailbox[22];
            d.failure_status = pmu_completion_visibility_v15_mailbox[23];
            d.installed_vector = pmu_completion_visibility_v15_mailbox[24];
            d.nvic_enabled_before_submit = pmu_completion_visibility_v15_mailbox[25];
            d.nvic_pending_after_initial_clear = pmu_completion_visibility_v15_mailbox[26];
            d.nvic_active_before_submit = pmu_completion_visibility_v15_mailbox[27];
            d.irq_triggered_before_submit = pmu_completion_visibility_v15_mailbox[28];
            d.nvic_pending_before_final_clear = pmu_completion_visibility_v15_mailbox[29];
            d.nvic_pending_after_final_clear = pmu_completion_visibility_v15_mailbox[30];
            d.nvic_active_after_cleanup = pmu_completion_visibility_v15_mailbox[31];
            d.irq_triggered_after_cleanup = pmu_completion_visibility_v15_mailbox[32];
            d.mailbox_valid = pmu_completion_visibility_v15_mailbox[33];
        }
#endif
#else
